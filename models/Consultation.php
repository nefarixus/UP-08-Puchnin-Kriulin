...
class Consultation {
    ...
    // Проверка корректности значений is_present и is_completed
    private function validateFlags() {
        if (!in_array($this->is_present, [0, 1])) {
            throw new Exception("Поле 'is_present' должно быть 0 или 1.");
        }
        if (!in_array($this->is_completed, [0, 1])) {
            throw new Exception("Поле 'is_completed' должно быть 0 или 1.");
        }
    }
